...
const BANK_KBANK   = "กสิกรไทย 184-3-50232-2";
const WALLET_NUM   = "081-327-1371 (ทรูมันนี่วอเล็ท)";
...
export default function App(){
  ...
  const publishGame=game=>{
    if(!game.title.trim()||game.scenes.length===0){notifShow("ต้องมีชื่อเกม และอย่างน้อย 1 ฉาก","err");return;}
    const prev=lsGet(LS_GAMES,games);const i=prev.findIndex(g=>g.id===game.id);
    let next;if(i>=0){next=[...prev];next[i]={...game,updatedAt:Date.now()};}else next=[{...game,createdAt:Date.now()},...prev];
    syncGames(next);notifShow("เผยแพร่เกมสำเร็จ! 🎉");setScreen("home");
  };
  ...
  const displayed=filterOn&&(prefs.genres.length||prefs.tones.length)
    ?games.filter(g=>g.tags&&(prefs.genres.some(t=>g.tags.includes(t))||prefs.tones.some(t=>g.tags.includes(t))))
    :games;
  ...
  return(
    <div style={{fontFamily:"Sarabun,sans-serif",minHeight:"100vh",background:"var(--color-background-tertiary)"}}>
      ...
      {screen==="profile" &&user&&<ProfileScreen user={user} games={games} setScreen={setScreen} newGame={newGame} editGame={editGame} delGame={delGame} updateProfile={updateProfile} notifShow={notifShow}/>}
...
// ── MusicPlayer — hidden iframe ────────────────────────────────────────────────
function MusicPlayer({url,autoplay=false,loop=false}){
  const vid=ytVideoId(url);
  if(!vid)return null;
  const src=`https://www.youtube.com/embed/${vid}?autoplay=${autoplay?1:0}&loop=${loop?1:0}&playlist=${vid}&controls=1&rel=0`;
  return(
    <div style={{position:"fixed",bottom:16,right:16,zIndex:80,background:"var(--color-background-primary)",borderRadius:14,boxShadow:"0 4px 20px rgba(0,0,0,0.15)",overflow:"hidden",width:200,border:"0.5px solid var(--color-border-secondary)"}}>
      <div style={{background:"#ff6b9d",padding:"4px 10px",display:"flex",alignItems:"center",gap:6}}>
        <span style={{fontSize:12}}>🎵</span>
        <span style={{fontSize:11,color:"#fff",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>เพลงประกอบเกม</span>
      </div>
      <iframe width="200" height="60" src={src} frameBorder="0" allow="autoplay; encrypted-media" style={{display:"block"}}/>
    </div>
  );
}
...
function CreateScreen({game,setGame,publish,setScreen,notifShow}){
  ...
  const S6=(
    <div style={{textAlign:"center",padding:"10px 0"}}>
      <div style={{borderRadius:16,height:180,overflow:"hidden",position:"relative",marginBottom:14,...prevBg}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:5,justifyContent:"center",marginBottom:14}}>{(game.tags||[]).map(t=><span key={t} style={{background:"#ff6b9d",color:"#fff",fontSize:11,padding:"2px 9px",borderRadius:20}}>{t}</span>)}</div>
      ...
      {game.title&&game.scenes.length>0
        ?<button onClick={()=>publish(game)} style={{...S.pink,width:"100%",padding:14,fontSize:16,fontWeight:700,borderRadius:12}}>{isEdit?"🔄 อัปเดตเกม!":"🚀 เผยแพร่เกม!"}</button>
        :<div style={{background:"var(--color-background-danger)",color:"var(--color-text-danger)",borderRadius:12,padding:"10px 14px",fontSize:13}}>ต้องมีชื่อเกม และอย่างน้อย 1 ฉาก</div>}
    </div>
  );
...
// ═══ PROFILE SCREEN ══════════════════════════════════════════════════════════
function ProfileScreen({user,games,setScreen,newGame,editGame,delGame,updateProfile,notifShow}){
                ...
                <div key={g.id} style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:14,overflow:"hidden"}}>
                  <div style={{display:"flex",gap:12,padding:"12px 14px",alignItems:"center"}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{margin:"0 0 2px",fontWeight:600,fontSize:14,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.title}</p>
                      <p style={{margin:0,fontSize:11,color:"var(--color-text-secondary)"}}>{g.scenes.length} ฉาก · {(g.minigames||[]).length} มินิเกม</p>
                      <div style={{display:"flex",gap:6,marginTop:4,alignItems:"center"}}>
                        <span style={{fontSize:10,padding:"2px 7px",borderRadius:8,background:ac?"#f0fdf4":"#fefce8",color:ac?"#16a34a":"#ca8a04",border:`0.5px solid ${ac?"#86efac":"#fde047"}`}}>{ac?`💰 รับรายได้`:`⏳ ${rc}/4 คน`}</span>
            ...
            <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-secondary)",borderRadius:14,padding:"14px 16px",marginBottom:12}}>
              ...
              <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <p style={{margin:0,fontSize:18,fontWeight:700,letterSpacing:2,color:"var(--color-text-primary)"}}>184-3-50232-2</p>
                </div>
                <button onClick={()=>copy("1843502322","bank")} style={{...S.pink,fontSize:12,padding:"6px 12px"}}>{copied==="bank"?"✅ คัดลอก":"📋 คัดลอก"}</button>
              </div>
            </div>
            ...
            <div style={{background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-secondary)",borderRadius:14,padding:"14px 16px",marginBottom:20}}>
              ...
              <div style={{background:"var(--color-background-secondary)",borderRadius:10,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                  ...
                  <p style={{margin:0,fontSize:18,fontWeight:700,letterSpacing:2,color:"var(--color-text-primary)"}}>081-327-1371</p>
                </div>
