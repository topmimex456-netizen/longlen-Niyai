...
const sendOTPEmail = async(toEmail, otp)=>{
  ...
  return window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
    to_email: toEmail,
    otp_code: otp,
    app_name:  "longlenNiyai",
    from_name: "ทีมงาน longlenNiyai",
  });
};
...
export default function App(){
  ...
  return(
    <div style={{fontFamily:"Sarabun,sans-serif",minHeight:"100vh",background:"var(--color-background-tertiary)"}}>
      <link href="https://fonts.googleapis.com/css2?family=Kanit:wght@500;600;700;800&family=Sarabun:wght@400;500;700&display=swap" rel="stylesheet"/>
...
// ═══ LEAF MARK (โลโก้ใบไม้ longlenNiyai) ══════════════════════════════════════
function LeafMark({size=26,glow=false}){
  return(
    <svg width={size} height={size} viewBox="0 0 32 32" fill="none" style={glow?{filter:"drop-shadow(0 0 7px rgba(126,217,87,0.7))"}:undefined}>
      <defs>
        <linearGradient id="lfg" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#a8e063"/><stop offset="0.55" stopColor="#7ed957"/><stop offset="1" stopColor="#3a9d3a"/>
        </linearGradient>
      </defs>
      <path d="M16 30 V15" stroke="#3a9d3a" strokeWidth="2.4" strokeLinecap="round"/>
      <path d="M16 17 C16 9 9 5 4 5 C4 13 9 17 16 17 Z" fill="url(#lfg)"/>
      <path d="M16 14 C16 6 23 2.5 29 3.5 C28 11 22 15 16 14 Z" fill="url(#lfg)"/>
      <path d="M7 7 C11 9 14 12 15.5 15.5" stroke="#2e7d32" strokeWidth="0.9" strokeLinecap="round" opacity="0.5"/>
      <path d="M26 6 C21 8 18 11 16.4 14" stroke="#2e7d32" strokeWidth="0.9" strokeLinecap="round" opacity="0.5"/>
    </svg>
  );
}
...
function Header({user,onLogout,setScreen,onPref}){
  const isAdminUser = user&&(user.email===ADMIN_EMAIL||user.username===ADMIN_USER);
  return(
    <header style={{background:"rgba(7,11,9,0.92)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",borderBottom:"1px solid rgba(126,217,87,0.18)",padding:"0 16px",display:"flex",alignItems:"center",justifyContent:"space-between",height:58,position:"sticky",top:0,zIndex:100}}>
      <button onClick={()=>setScreen("home")} style={{background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:9}}>
        <LeafMark size={26} glow/>
        <span style={{fontFamily:"Kanit,Sarabun,sans-serif",fontWeight:700,fontSize:19,color:"#eaf3ea",letterSpacing:0.2}}>longlen<span style={{color:"#7ed957"}}>Niyai</span></span>
      </button>
      <div style={{display:"flex",gap:6,alignItems:"center"}}>
        <button onClick={onPref} title="ความชอบ" style={{background:"none",border:"1px solid rgba(126,217,87,0.3)",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:13,color:"#cfe6cf"}}>🎨</button>
        {user?(
          <>
            <button onClick={()=>setScreen("topup")} style={{background:"linear-gradient(135deg,#52c234,#7ed957)",border:"none",color:"#06140a",fontWeight:700,borderRadius:20,padding:"5px 11px",cursor:"pointer",fontSize:12,fontFamily:"Sarabun,sans-serif",display:"flex",alignItems:"center",gap:4,boxShadow:"0 2px 12px rgba(126,217,87,0.3)"}}>
              ⭐{user.points} <span style={{fontSize:11,opacity:0.85}}>+เติม</span>
              {user.isMember&&<span style={{background:"rgba(6,20,10,0.25)",borderRadius:10,padding:"0 5px",fontSize:10}}>💎VIP</span>}
            </button>
            <button onClick={()=>setScreen("profile")} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(126,217,87,0.25)",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:13,color:"#eaf3ea",display:"flex",alignItems:"center",gap:5}}>
            ...
            <button onClick={onLogout} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"#7d927d"}}>ออก</button>
          </>
        ):<button onClick={()=>setScreen("auth")} style={{background:"linear-gradient(135deg,#52c234,#7ed957)",border:"none",color:"#06140a",fontWeight:700,borderRadius:20,padding:"7px 16px",cursor:"pointer",fontFamily:"Sarabun,sans-serif",fontSize:13,boxShadow:"0 2px 12px rgba(126,217,87,0.3)"}}>เข้าสู่ระบบ</button>}
      </div>
    </header>
  );
}
...
// ═══ HOME SCREEN (Landing — ดำตัดเขียว longlenNiyai) ══════════════════════════
function HomeScreen({games,allGames,startPlay,newGame,user,setScreen,prefs,filterOn,setFilterOn,onPref,editGame,delGame}){
  ...
  const scrollToGames=()=>{const el=document.getElementById("ll-games");if(el)el.scrollIntoView({behavior:"smooth"});};
  // อนุภาคใบไม้/ประกาย ลอยขึ้น
  const particles=Array.from({length:16},(_,i)=>({
    id:i, left:Math.round((i*61+13)%100), size:8+((i*7)%10), dur:9+((i*5)%9),
    delay:-((i*1.7)%9), char:["🌿","✨","🍃","💚","⭐"][i%5], op:0.25+((i%4)*0.12)
  }));
  const pills=[
    {icon:"🎮",t:"เล่นได้ทันที",s:"ไม่ต้องโหลด"},
    {icon:"✏️",t:"สร้างง่าย",s:"ไม่ต้องเขียนโค้ด"},
    {icon:"💡",t:"อิสระเต็มที่",s:"ออกแบบเองได้หมด"},
    {icon:"👥",t:"คอมมูนิตี้",s:"แชร์ & เติบโต"},
  ];
  return(
    <div className="ll-home">
      <style>{`
        .ll-home{--ink:#070b09;--leaf:#7ed957;--leaf2:#52c234;--leafd:#2e7d32;--mist:#9db5a6;--paper:#eaf3ea;--pink:#ff7eae;
          position:relative;min-height:100vh;font-family:Sarabun,sans-serif;color:var(--paper);overflow:hidden;
          background:
            radial-gradient(950px 520px at 80% -10%, rgba(126,217,87,.16), transparent 60%),
            radial-gradient(720px 620px at 4% 8%, rgba(94,46,140,.22), transparent 62%),
            radial-gradient(640px 520px at 50% 116%, rgba(126,217,87,.10), transparent 60%),
            #060a08;}
        .ll-home .wrap{position:relative;z-index:3;max-width:1080px;margin:0 auto;padding:8px 18px 64px;}
        .ll-disp{font-family:Kanit,Sarabun,sans-serif;}
        .ll-grain{position:absolute;inset:0;z-index:1;pointer-events:none;
          background-image:radial-gradient(rgba(255,255,255,.05) 1px,transparent 1px);background-size:42px 42px;opacity:.5;}
        .ll-particles{position:absolute;inset:0;z-index:2;pointer-events:none;overflow:hidden;}
        .ll-p{position:absolute;bottom:-30px;animation:ll-rise linear infinite;}
        @keyframes ll-rise{0%{transform:translateY(0) rotate(0);opacity:0}
          10%{opacity:var(--o,.4)}90%{opacity:var(--o,.4)}100%{transform:translateY(-112vh) rotate(220deg);opacity:0}}
        .ll-hero{display:grid;grid-template-columns:1.15fr .85fr;gap:30px;align-items:center;padding:22px 0 8px;}
        .ll-h1{font-family:Kanit,Sarabun,sans-serif;font-weight:800;font-size:clamp(30px,5.4vw,52px);line-height:1.04;margin:14px 0 0;letter-spacing:.3px;}
        .ll-h1 .g{background:linear-gradient(120deg,#a8e063,#7ed957,#39c66d);-webkit-background-clip:text;background-clip:text;color:transparent;}
        .ll-sub{color:var(--mist);font-size:clamp(14px,1.7vw,17px);line-height:1.6;margin:16px 0 0;max-width:30em;}
        .ll-pills{display:flex;flex-wrap:wrap;gap:9px;margin:22px 0 0;}
        .ll-pill{display:flex;align-items:center;gap:9px;background:rgba(126,217,87,.07);border:1px solid rgba(126,217,87,.22);
          border-radius:14px;padding:8px 13px;}
        .ll-pill b{font-size:13px;font-weight:700;color:var(--paper);font-family:Sarabun}
        .ll-pill span{font-size:11px;color:var(--mist)}
        .ll-cta{display:flex;flex-wrap:wrap;gap:12px;margin:26px 0 0;}
        .ll-btn{font-family:Kanit,Sarabun,sans-serif;font-weight:700;border-radius:16px;cursor:pointer;font-size:16px;border:none;transition:transform .15s,box-shadow .15s;}
        .ll-btn-primary{background:linear-gradient(135deg,#52c234,#7ed957);color:#06140a;padding:15px 26px;box-shadow:0 8px 28px rgba(126,217,87,.34);}
        .ll-btn-primary:hover{transform:translateY(-2px);box-shadow:0 12px 36px rgba(126,217,87,.5);}
        .ll-btn-ghost{background:rgba(255,255,255,.04);color:var(--paper);border:1.5px solid rgba(126,217,87,.35);padding:15px 24px;}
        .ll-btn-ghost:hover{border-color:var(--leaf);background:rgba(126,217,87,.1);}
        /* preview card */
        .ll-preview{position:relative;border-radius:26px;padding:14px;background:linear-gradient(160deg,rgba(20,30,24,.9),rgba(9,14,11,.92));
          border:1px solid rgba(126,217,87,.28);box-shadow:0 30px 80px rgba(0,0,0,.55),0 0 0 1px rgba(126,217,87,.05) inset;}
        .ll-screen{border-radius:18px;overflow:hidden;background:linear-gradient(160deg,#243a52,#101826);position:relative;height:300px;}
        .ll-scene-emoji{position:absolute;right:14px;bottom:0;font-size:120px;filter:drop-shadow(0 6px 16px rgba(0,0,0,.5));}
        .ll-bubble{position:absolute;left:14px;top:16px;right:64px;background:rgba(10,16,13,.82);border:1px solid rgba(126,217,87,.3);
          color:#eaf3ea;font-size:13px;padding:10px 13px;border-radius:14px;backdrop-filter:blur(4px);}
        .ll-choices{position:absolute;left:14px;right:14px;bottom:14px;display:flex;flex-direction:column;gap:8px;}
        .ll-choice{background:rgba(126,217,87,.14);border:1px solid rgba(126,217,87,.35);color:#eaf3ea;font-size:13px;
          padding:9px 13px;border-radius:12px;display:flex;align-items:center;gap:8px;}
        .ll-flow{display:flex;align-items:center;justify-content:center;gap:6px;margin-top:12px;flex-wrap:wrap;}
        .ll-node{font-size:11px;font-weight:700;font-family:Kanit,Sarabun;color:#06140a;background:linear-gradient(135deg,#7ed957,#52c234);
          padding:5px 11px;border-radius:9px;}
        .ll-node.alt{background:rgba(126,217,87,.12);color:#bfe6b3;border:1px solid rgba(126,217,87,.3);}
        .ll-arrow{color:var(--leaf);font-size:13px}
        /* feature duo */
        .ll-duo{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:46px 0 0;}
        .ll-fcard{background:linear-gradient(160deg,rgba(18,27,21,.7),rgba(9,14,11,.7));border:1px solid rgba(126,217,87,.18);
          border-radius:20px;padding:22px;transition:border-color .2s,transform .2s;}
        .ll-fcard:hover{border-color:rgba(126,217,87,.45);transform:translateY(-3px);}
        .ll-fcard h3{font-family:Kanit,Sarabun;font-weight:700;font-size:19px;margin:10px 0 6px;color:var(--paper);}
        .ll-fcard p{color:var(--mist);font-size:13.5px;line-height:1.55;margin:0;}
        .ll-ficon{width:46px;height:46px;border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:24px;
          background:rgba(126,217,87,.12);border:1px solid rgba(126,217,87,.28);}
        /* section */
        .ll-sec{display:flex;justify-content:space-between;align-items:center;margin:48px 0 16px;}
        .ll-sec h2{font-family:Kanit,Sarabun;font-weight:700;font-size:21px;margin:0;color:var(--paper);}
        .ll-sec .eb{color:var(--leaf);font-size:12px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;}
        .ll-filterbtn{font-family:Sarabun;font-size:12px;border-radius:20px;padding:6px 14px;cursor:pointer;}
        .ll-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(172px,1fr));gap:14px;}
        .ll-empty{text-align:center;padding:48px 20px;background:rgba(126,217,87,.05);border:1px dashed rgba(126,217,87,.28);border-radius:20px;}
        @media(max-width:760px){.ll-hero{grid-template-columns:1fr;gap:20px}.ll-preview{order:-1;max-width:420px;margin:0 auto}.ll-duo{grid-template-columns:1fr}}
        @media(prefers-reduced-motion:reduce){.ll-p{animation:none;display:none}.ll-btn{transition:none}}
      `}</style>

      <div className="ll-grain"/>
      <div className="ll-particles" aria-hidden="true">
        {particles.map(p=>(
          <div key={p.id} className="ll-p" style={{left:`${p.left}%`,fontSize:p.size+6,animationDuration:`${p.dur}s`,animationDelay:`${p.delay}s`,["--o"]:p.op}}>{p.char}</div>
        ))}
      </div>
      ...
      <div className="wrap">
        ...
        <div className="ll-hero">
          <div>
            <div style={{display:"inline-flex",alignItems:"center",gap:9,background:"rgba(126,217,87,.1)",border:"1px solid rgba(126,217,87,.3)",borderRadius:30,padding:"6px 14px 6px 10px"}}>
              <LeafMark size={20} glow/>
              <span className="ll-disp" style={{fontWeight:700,fontSize:14,letterSpacing:.4}}>longlen<span style={{color:"#7ed957"}}>Niyai</span></span>
            </div>
            <h1 className="ll-h1">เว็บเล่นเกม & สร้างเกมจีบ<br/><span className="g">ครบจบในที่เดียว!</span></h1>
            <p className="ll-sub">เล่นเกมจีบสาว จีบหนุ่ม หรือสร้างเกมจีบของคุณเอง ง่ายๆ แค่มีมือถือเครื่องเดียว เริ่มได้ฟรีวันนี้</p>
            <div className="ll-pills">
              {pills.map(p=>(
                <div key={p.t} className="ll-pill"><span style={{fontSize:18}}>{p.icon}</span><div><b>{p.t}</b><br/><span>{p.s}</span></div></div>
              ))}
            </div>
            <div className="ll-cta">
              <button className="ll-btn ll-btn-primary" onClick={newGame}>✏️ สร้างเกมของคุณ</button>
              <button className="ll-btn ll-btn-ghost" onClick={user?scrollToGames:()=>setScreen("auth")}>{user?"🎮 เริ่มเล่นเลย":"สมัครรับ 100 พอยต์ฟรี ⭐"}</button>
            </div>
          </div>

          {/* PREVIEW (จำลองหน้าเล่นเกมแบบโปสเตอร์) */}
          <div className="ll-preview">
            <div className="ll-screen">
              <div className="ll-scene-emoji">🧑‍🎤</div>
              <div className="ll-bubble">สวัสดี... เธอชื่ออะไรเหรอ? 💬</div>
              <div className="ll-choices">
                <div className="ll-choice">💚 เราเป็นเพื่อนกันนะ!</div>
                <div className="ll-choice" style={{background:"rgba(255,126,174,.14)",borderColor:"rgba(255,126,174,.4)"}}>💗 เธอน่ารักจัง &gt;///&lt;</div>
              </div>
            </div>
            <div className="ll-flow">
              <span className="ll-node">เริ่มต้น</span><span className="ll-arrow">→</span>
              <span className="ll-node alt">บทสนทนา</span><span className="ll-arrow">→</span>
              <span className="ll-node alt">ตัวเลือก</span><span className="ll-arrow">→</span>
              <span className="ll-node alt">จบเนื้อเรื่อง</span>
            </div>
          </div>
        </div>
        ...
        <div className="ll-duo">
          <div className="ll-fcard">
            <div className="ll-ficon">▶️</div>
            <h3>เล่นเกมจีบ ได้ทุกที่ ทุกเวลา</h3>
            <p>รวมเกมจีบสาว & จีบหนุ่มคุณภาพจากนักพัฒนาทั่วโลก เลือกเล่นได้ตามสไตล์ที่คุณชอบ</p>
          </div>
          <div className="ll-fcard">
            <div className="ll-ficon">✨</div>
            <h3>สร้างเกมจีบ ของคุณเอง</h3>
            <p>ระบบสร้างเกมสุดง่าย ลากวางได้เลย! ไม่ต้องเขียนโค้ด ก็สร้างเกมจีบในฝันได้</p>
          </div>
        </div>

        {/* PREF CHIPS */}
        {hasPref&&(
          <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginTop:34,background:"rgba(126,217,87,.06)",border:"1px solid rgba(126,217,87,.18)",borderRadius:14,padding:"9px 14px"}}>
            <span style={{fontSize:12,color:"#9db5a6"}}>🎨 ความชอบ:</span>
            {[...prefs.genres,...prefs.tones].slice(0,6).map(t=><span key={t} style={{background:"linear-gradient(135deg,#52c234,#7ed957)",color:"#06140a",fontWeight:600,fontSize:11,padding:"3px 10px",borderRadius:20}}>{t}</span>)}
            <button onClick={onPref} style={{background:"none",border:"none",fontSize:12,color:"#7ed957",cursor:"pointer",marginLeft:"auto"}}>แก้ไข</button>
          </div>
        )}

        {/* GAMES SECTION */}
        <div id="ll-games" className="ll-sec">
          <div>
            <div className="eb">เกมทั้งหมด</div>
            <h2>{filterOn&&hasPref?"เกมตามความชอบของคุณ":"เลือกเกมที่อยากเล่น"}{filterOn&&hasPref&&<span style={{fontSize:13,color:"#9db5a6",fontWeight:400,marginLeft:8}}>({games.length}/{allGames.length})</span>}</h2>
          </div>
          {hasPref&&<button className="ll-filterbtn" onClick={()=>setFilterOn(f=>!f)} style={{background:filterOn?"linear-gradient(135deg,#52c234,#7ed957)":"rgba(255,255,255,.04)",border:filterOn?"none":"1px solid rgba(126,217,87,.3)",color:filterOn?"#06140a":"#cfe6cf",fontWeight:filterOn?700:400}}>{filterOn?"🔍 กรองอยู่":"กรองตามชอบ"}</button>}
        </div>

        {games.length===0?(
          <div className="ll-empty">
            <div style={{fontSize:42,marginBottom:10}}>🌱</div>
            <p style={{color:"#9db5a6",margin:"0 0 16px"}}>{filterOn&&hasPref?"ยังไม่มีเกมที่ตรงความชอบ":"ยังไม่มีเกม — เป็นคนแรกที่ปลูกเกมของคุณเลย!"}</p>
            <button className="ll-btn ll-btn-primary" style={{fontSize:14,padding:"11px 22px"}} onClick={filterOn&&hasPref?()=>setFilterOn(false):newGame}>{filterOn&&hasPref?"ดูทั้งหมด":"สร้างเกมแรก 🎉"}</button>
          </div>
        ):(
        ...
        {/* FOOTER CTA */}
        <div style={{textAlign:"center",marginTop:56,padding:"36px 20px",background:"radial-gradient(600px 200px at 50% 0%,rgba(126,217,87,.14),transparent 70%)",borderTop:"1px solid rgba(126,217,87,.14)"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:8}}>
            <LeafMark size={28} glow/>
            <span className="ll-disp" style={{fontWeight:700,fontSize:22}}>longlen<span style={{color:"#7ed957"}}>Niyai</span></span>
          </div>
          <p className="ll-disp" style={{fontSize:18,fontWeight:600,margin:"0 0 4px"}}>มาร่วมเป็นส่วนหนึ่งของโลกแห่งความรักและจินตนาการ!</p>
          <p style={{color:"#9db5a6",fontSize:13,margin:"0 0 18px"}}>💚 เล่นฟรี · ✏️ สร้างฟรี · 💗 เป็นตัวคุณ</p>
          <button className="ll-btn ll-btn-primary" onClick={user?newGame:()=>setScreen("auth")}>เข้าร่วมฟรี! ตอนนี้</button>
        </div>
      </div>
    </div>
  );
...
// ═══ SPLASH SCREEN ════════════════════════════════════════════════════════════
const LOGO_URL = "https://raw.githubusercontent.com/longlen-Niyai/longlen-Niyai/main/67730.png";
...
function SplashScreen({onDone}){
  ...
  return(
    <div style={{
      ...
      background:"radial-gradient(700px 500px at 50% 35%, rgba(126,217,87,.18), transparent 60%), radial-gradient(600px 600px at 80% 90%, rgba(94,46,140,.25), transparent 60%), #060a08",
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      transition:"opacity 0.6s ease",
      opacity:phase==="fadeout"?0:1,fontFamily:"Sarabun,sans-serif",overflow:"hidden",
    }}>
      {/* ประกายลอย */}
      {Array.from({length:10}).map((_,i)=>(
        <div key={i} style={{position:"absolute",bottom:-20,left:`${(i*53+9)%100}%`,fontSize:12+(i%4)*5,opacity:.5,
          animation:`splash-rise ${7+(i%5)}s linear infinite`,animationDelay:`${-(i*1.3)}s`}}>{["🌿","✨","🍃","💚"][i%4]}</div>
      ))}
      ...
      <div style={{
        ...
        animation:"splash-pop 0.6s cubic-bezier(0.34,1.56,0.64,1) forwards",
      }}>
        <div style={{filter:"drop-shadow(0 0 24px rgba(126,217,87,0.65))"}}>
          <LeafMark size={92} glow/>
        </div>
      </div>

      {/* App name */}
      <h1 style={{fontFamily:"Kanit,Sarabun,sans-serif",fontSize:34,fontWeight:700,margin:"0 0 6px",letterSpacing:.5,color:"#eaf3ea"}}>
        longlen<span style={{color:"#7ed957"}}>Niyai</span>
      </h1>
      <p style={{color:"#9db5a6",fontSize:14,margin:"0 0 34px"}}>
        เล่นเกมจีบ & สร้างเกมจีบ ครบจบในที่เดียว
      </p>
      ...
      <div style={{display:"flex",gap:8}}>
        {[0,1,2].map(i=>(
          <div key={i} style={{
            width:8,height:8,borderRadius:"50%",background:"#7ed957",
            animation:`dot-bounce 1.2s ease-in-out infinite`,
            animationDelay:`${i*0.2}s`,boxShadow:"0 0 8px rgba(126,217,87,.6)",
          }}/>
        ))}
      </div>
      ...
      <style>{`
        ...
        @media(prefers-reduced-motion:reduce){
          [style*="splash-rise"]{display:none!important}
        }
      `}</style>
    </div>
  );
...
// BGM handled in MusicBar component
...
function _playSfx(src){ if(src){ playAudioSrc(src,"sfx",false,0.8); } }
function _playVoice(src){ if(src){ playAudioSrc(src,"voice",false,1.0); } }
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {[["#ffd6e7","#ffb3cf"],["#d6eaff","#b5d4ff"],["#d4f5d4","#a8e8a8"],["#ffe8cc","#ffd4a0"],["#e8d4ff","#d0a8ff"],["#2d1b69","#11092e"],["#1a1a2e","#0a0a1e"],["#fff9c4","#fff176"]].map(([c1,c2],_i)=>(
              <button key={_i} onClick={()=>set({type:"gradient",c1,c2})} style={{width:32,height:32,borderRadius:8,background:`linear-gradient(135deg,${c1},${c2})`,border:`2px solid ${cv.c1===c1?"#ff6b9d":"transparent"}`,cursor:"pointer"}}/>
            ))}
          </div>
        </div>
        {/* Text overlay */}
...
function MG_Memory({onFinish}){
  const em=["💕","🌸","⭐","🎀","🍓","🦋"];
  const [cards,setCards]=useState(()=>[...em,...em].sort((_a,_b)=>Math.random()-.5).map((e,i)=>({id:i,e,f:false,m:false})));
    ...
    <div style={{maxWidth:500,margin:"0 auto",padding:16,paddingBottom:40}}>
      ...
      {user&&<>
        ...
        <div style={{display:"flex",gap:4,marginBottom:18}}>
          {["เลือกแพ็กเกจ","โอนเงิน","แนบสลิป"].map((_s,i)=>(
...
// ═══ ADMIN SCREEN ════════════════════════════════════════════════════════════
function AdminScreen({allUsers,_requests,approveTopup,rejectTopup,adminGivePoints,setScreen,notifShow}){
